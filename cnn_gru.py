...
from tensorflow.keras.layers import Conv1D, GRU, Dense, Dropout, Bidirectional, Reshape
...
# Reshape for CNN + GRU input (add channels dimension)
...
model = Sequential([
    ...
    Dropout(0.3),
    # Reshape the output to match the expected shape for GRU
    Reshape((X_train.shape[1]-4, 128)),  # Adjust dimensions to fit GRU (timesteps, features)
    # GRU Layer
...
model_checkpoint = ModelCheckpoint('best_model_sitar_gru.keras', save_best_only=True)
...
model.save('sitar_note_cnn_gru.keras')
